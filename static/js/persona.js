document.addEventListener('DOMContentLoaded',function(){
    ...
    if(addBtn)addBtn.onclick=function(){
        ...
        document.getElementById('persona-model').value='';
    ...
    if(saveBtn)saveBtn.onclick=function(){
        ...
        var d={name:document.getElementById('persona-name').value.trim(),description:document.getElementById('persona-description').value.trim(),system_prompt:document.getElementById('persona-system-prompt').value,model:document.getElementById('persona-model').value.trim()};
    ...
    function loadPersonas(){
        fetch('/api/personas').then(function(r){return r.json()}).then(function(ps){
            ...
            grid.innerHTML=ps.map(function(p){
                ...
                return '<div class="persona-card'+(p.active?' active':'')+'"><div class="p-name">'+esc(p.name)+(p.active?' <span class="p-badge">当前激活</span>':'')+'</div>'+(p.description?'<div class="p-desc">'+esc(p.description)+'</div>':'')+(p.model?'<span class="p-model">'+esc(p.model)+'</span>':'')+'<div class="p-preview">'+esc(pv)+'</div><div class="p-actions">'+(p.active?'':'<button class="btn btn-primary btn-sm" onclick="activatePersona(\''+p.id+'\')"><i class="fas fa-check"></i> 激活</button>')+'<button class="btn btn-secondary btn-sm" onclick="editPersona(\''+p.id+'\')"><i class="fas fa-edit"></i> 编辑</button><button class="btn btn-danger btn-sm" onclick="deletePersona(\''+p.id+'\')"><i class="fas fa-trash"></i></button></div></div>'
            }).join('');
        }).catch(function(){grid.innerHTML='<div class="persona-empty">加载失败</div>'});
    }
    window.loadPersonas=loadPersonas;

    window.activatePersona=function(id){fetch('/api/personas/'+id+'/activate',{method:'POST'}).then(function(){loadPersonas();if(typeof showToast==='function')showToast('人格已切换','success')})};
    window.editPersona=function(id){fetch('/api/personas').then(function(r){return r.json()}).then(function(all){var p=all.find(function(x){return x.id===id});if(!p)return;document.getElementById('persona-modal-title').textContent='编辑人格';document.getElementById('persona-id').value=p.id;document.getElementById('persona-name').value=p.name;document.getElementById('persona-description').value=p.description||'';document.getElementById('persona-system-prompt').value=p.system_prompt||'';document.getElementById('persona-model').value=p.model||'';modal.classList.add('show')})};
